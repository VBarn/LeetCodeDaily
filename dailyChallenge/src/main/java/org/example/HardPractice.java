...
public class HardPractice {
    ...
    class Pair implements Comparable{
        int canReach;
        int inSteps;
        public Pair(int canReach,int inSteps){
        ...
        public int compareTo(Object o) {
            if(o instanceof Pair){
                return ((Pair) o).canReach==canReach?Integer.compare(((Pair) o).inSteps,inSteps):Integer.compare(((Pair) o).canReach,canReach);
            }
            return -1;
        }
    }
    ...
    public boolean canCross(int[] stones) {
        for(int i=2;i<stones.length-1;i++)
            if(stones[i+1]>2*stones[i])
                return false;
        PriorityQueue<Pair> pakya=new PriorityQueue<>((a,b)->Integer.compare(b.canReach,a.canReach));
        HashSet<String> hs=new HashSet<>();
        pakya.add(new Pair(1,1));
        while(!pakya.isEmpty()){
            Pair cur=pakya.poll();
            if(!hs.add(cur.canReach+"$"+cur.inSteps))
                continue;
            int temp=Arrays.binarySearch(stones,cur.canReach);
            if(temp==stones.length-1)
                return true;
            if(temp>-1){
                if(cur.inSteps!=1)
                    pakya.add(new Pair(cur.canReach+ cur.inSteps-1,cur.inSteps-1));
                pakya.add(new Pair(cur.canReach+ cur.inSteps,cur.inSteps));
                pakya.add(new Pair(cur.canReach+ cur.inSteps+1,cur.inSteps+1));
            }
        }
        return false;
